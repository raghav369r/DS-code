//in bst total nodes ,internal,external(leaf),mirror image of bst
#include<stdio.h>
#include<stdlib.h>
struct bst {
	struct bst *left;
	...
	while(1)
	{
		...
		switch(opt)
		{
			...
			case 2: // Preorder
		        ...
		        case 3:
		              t=totalnodes(root);
		              printf("total no of nodes in bst=%d",t); 
		              break;
		         case 4:
		              t=total_internal_nodes(root);
		              printf("total no of internal nodes in bst=%d",t); 
		              break;
		         case 5:
		             t=external(root);
		             printf("total no of external nodes=%d",t);
		             break;
		         case 6:
		             mirror(root);
		             break;
